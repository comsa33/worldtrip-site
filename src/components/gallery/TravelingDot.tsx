...
export function TravelingDot() {
  ...
  useEffect(() => {
    ...
    const follow = (host: HTMLElement) => {
      ...
      const tick = () => {
        ...
        if (carry !== lastCarry) {
          ...
          const wasCaret = lastCarry.startsWith('caret');
          const isCaret = carry.startsWith('caret');
          if (isCaret && ball) {
            // a cursor: stood up out of the dot the first time, then held
            caretVars(host, s.size);
            if (!wasCaret) {
              ball.removeAttribute('data-squish');
              ball.removeAttribute('data-drop');
              ...
              window.setTimeout(() => {
                if (ball.hasAttribute('data-caret-in')) {
    ...
    /** The caret the dot stands up into while it writes: the height of the
     *  line the seat is on, and this thin. */
    const caretVars = (host: HTMLElement, size: number) => {
      if (!ball) return;
