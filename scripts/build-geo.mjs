// Builds src/data/landDots.json and src/data/worldBorders.json from Natural Earth 110m (world-atlas).
...
// --- land dots: ~1.25° equal-area-ish grid, only where land is
...
for (let lat = -88; lat <= 88; lat += STEP) {
  ...
  for (let j = 0; j < n; j++) {
    ...
    if (geoContains(landFeature, [lng, lat])) dots.push(+lat.toFixed(2), +lng.toFixed(2));
  }
}
...
// --- visited countries: rings for highlight
...
for (const code of visited) {
  ...
  if (!f) { console.warn('no feature for', code); continue; }
  const g = f.geometry;
...
console.log(`landDots: ${dots.length / 2} dots · borders: ${borders.length} lines · countries: ${Object.keys(byCode).length}`);
